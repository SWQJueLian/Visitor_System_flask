...
def invite_get_all(filter_data):
    """根据搜索关键词、时间、员工id列出邀请"""
    filter_kw = [
        Invite.employee_id == filter_data['employee_id']
    ]
    # 如果有时间日期过滤就加上。
    # 这里要用来做下拉刷新和上拉瀑布流加载
    ...
    if filter_data.get('keyword'):
        filter_or_kw = [Invite.visitor_name.contains(filter_data.get('keyword')),
                        Invite.visitor_mobile.contains(filter_data.get('keyword'))]

    # 减少不必要的字段查询
    invite_list = db.session.scalars(
        sa.select(Invite).where(*filter_kw, sa.or_(*filter_or_kw)).limit(filter_data.get('limit', 20)).options(
            load_only(Invite.id, Invite.visitor_name, Invite.visitor_mobile, Invite.visit_date, Invite.created_at,
                      Invite.status))).all()

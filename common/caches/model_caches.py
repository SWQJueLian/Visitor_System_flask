...
class InviteCache(BaseModelCache):
    ...
    def get(self, serializer_rules=(), serializer_only=(), *args, **kwargs):
        ...
        if not invite_info:
            ...
            invite_info = invite.to_dict(rules=serializer_rules, only=serializer_only)  # 调用sqlalchemy_serializer的方法序列化对象。
    ...
    def get(self, serializer_rules=(), serializer_only=(), *args, **kwargs):
        ...
        if not employee_info:
            ...
            if employee is None:
                ...
                # 5.如果能从数据库中读取数据，则进行缓存回填
                employee_info = employee.to_dict(rules=serializer_rules, only=serializer_only)  # 调用sqlalchemy_serializer的方法序列化对象。
